...
| NEON data (DP1.10003.001, DP1.10022.001, RELEASE-2026) via neon-avu-eval dataset cards and validated results | https://data.neonscience.org | CC BY 4.0 | Bench states and silver labels. The seven dataset cards and `results/validated.json` (the evaluation's own output over NEON metadata) are vendored under `tests/fixtures/neon-avu-eval/` so CI can ingest them; `MESA_ANYJEV_EVAL_ROOT` points at a full checkout instead. |
