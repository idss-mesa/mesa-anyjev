#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt from the mesa-anyjev docs/ OKF bundle.
...
(https://idss-mesa.github.io/mesa-anyjev/llms.txt). They are committed; CI fails
...
def main():
    base = site_url()
    lines = [
        "# mesa-anyjev documentation",
        "",
        "> mesa-anyjev makes MESA's ontology and schema choices declarative and "
        "calibrated: AnyJev typed decisions (choice, yes/no, score) over mesa-mcp's "
        "OBO/OLS ontology tools, with every decision recorded in a provenance "
        "sidecar next to the mesa-ducklake AVU history, and hosted Jev through "
        "MotherDuck prompt_jev as a policy-gated third provider. This documentation "
        "covers installation, configuration, the decision graph, calibration "
        "levels and the write policy, provenance, hosted providers and development. "
        "The source repository is an Open Knowledge Format (OKF v0.2) bundle: every "
        "page carries YAML frontmatter with type, provenance (generated/sources), "
        "and lifecycle (status/stale_after) fields.",
        "",
        ...
        f"{base}getting-started/install/index.md. Agent guide: "
        f"{base}about/ai-agents/",
        "",
        "For the decisions and verified facts behind the design, read DESIGN.md "
        "and RESEARCH.md in the repository; for behaviour, run mesa-anyjev and "
        "read its provenance sidecar.",
        "",
    ]
    full = [
        "# mesa-anyjev documentation — full corpus",
        "",
